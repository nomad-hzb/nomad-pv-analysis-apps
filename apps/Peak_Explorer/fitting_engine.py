...
class PeakDetector:
        ...
        **kwargs : dict
        ...
        for i, peak_idx in enumerate(peaks):
            ...
            peak_data = {
                ...
                "prominence": properties.get("prominences", [None])[i]
            ...
            try:
                ...
                peak_data["left_base"] = (
                    wavelengths[int(left_ips[0])]
                    if left_ips[0] < len(wavelengths)
                    else wavelengths[0]
                )
                peak_data["right_base"] = (
                    wavelengths[int(right_ips[0])]
                    if right_ips[0] < len(wavelengths)
                    else wavelengths[-1]
                )
    ...
    def detect_peaks_advanced(
        ...
        for i, peak_idx in enumerate(refined_peaks):
            peak_data = self._calculate_peak_properties(wavelengths, intensities, peak_idx)
            peak_info.append(peak_data)
    ...
    def _find_peak_base(self, intensities, peak_idx):
        ...
        right_idx = peak_idx
        while (
            right_idx < len(intensities) - 1 and intensities[right_idx + 1] < intensities[right_idx]
        ):
            right_idx += 1
    ...
    def _apply_smart_init(self, base_params, previous_result):
        ...
        for i, peak_model in enumerate(smart_params["peak_models"]):
            ...
            if center_key in fitted_params:
                ...
                debug_print(
                    f"Smart init p{i}: center {old_center:.2f} -> {peak_model['center']:.2f}",
                    "FITTING",
                )
    ...
    def extract_peak_parameters(self, fit_results):
        ...
        for idx, result in fit_results.items():
            ...
            for param_name, param_data in result["parameters"].items():
                if any(
                    x in param_name for x in ["center", "amplitude", "sigma", "gamma", "height"]
                ):
...
class FittingEngine:
    ...
    def detect_peaks(self, wavelengths, intensities, **detection_params):
        ...
        **detection_params : dict
        ...
        peaks = self.peak_detection.detect_peaks(wavelengths, intensities, **detection_params)

        debug_print(f"Detected {len(peaks)} peaks", "FITTING")
        return peaks

    def create_fit_parameters(
        self, peak_models, background_model="Linear", poly_degree=2, center_bound=None, **kwargs
    ):
        """
        ...
        debug_print(
            f"Created fit parameters: {len(peak_models)} peaks, bg={background_model}", "FITTING"
        )
    ...
    def fit_current_spectrum(self, wavelengths, intensities):
        ...
        result = self.fitting_models.fit_spectrum(wavelengths, intensities, self.fit_params)
    ...
    def fit_all_spectra(
        ...
        debug_print(
            f"Starting batch fit: {len(time_subset)} spectra, workers={max_workers}, smart_init={use_smart_init}",
            "FITTING",
        )
        ...
        debug_print(
            f"Batch fitting complete: {successful_fits}/{len(results)} successful", "FITTING"
        )
        ...
        if failed:
            ...
            for idx, r in sorted(failed.items()):
                if r is None:
                    ...
                    debug_print(
                        f"  idx={idx} t={r.get('time', '?'):.3f}: exception: {r['error']}",
                        "FITTING",
                    )
                else:
                    ...
                    debug_print(
                        f"  idx={idx} t={r.get('time', '?'):.3f}: lmfit success=False "
                        f"(R²={r2:.4f}, redchi={redchi:.4f}{bound_str})",
                        "FITTING",
                    )
