...
class PlotManager:
    ...
    def create_plot(
        ...
        **kwargs,
        ...
        try:
            if plot_type == "splom":
                return self.create_scatter_matrix(
                    plot_data, variables, algorithm=algorithm, **kwargs
                )
            elif plot_type == "parallel":
                return self.create_parallel_coordinates(
                    plot_data, variables, algorithm=algorithm, **kwargs
                )
            elif plot_type == "distributions":
                return self.create_distribution_plots(
                    plot_data, variables, algorithm=algorithm, **kwargs
                )
                ...
                return self.create_quality_plots(
                    plot_data, variables, algorithm=algorithm, **kwargs
                )
            else:
    ...
    def create_scatter_matrix(
        self, data: pd.DataFrame, variables: List[Variable], **kwargs
    ) -> go.Figure:
        ...
        fig = go.Figure(
            data=go.Splom(
                dimensions=[
                    {"label": var.name, "values": splom_data[var.name]} for var in numeric_vars
                ],
                text=data.index,
                marker=dict(
    ...
    def create_parallel_coordinates(
        ...
        for var in variables:
            ...
            if var.type == VariableType.CATEGORICAL:
                ...
                dimension = dict(
                    label=var.name, values=numeric_col, range=[numeric_col.min(), numeric_col.max()]
                )
        ...
        fig.update_layout(
            title=f"Parallel Coordinates Plot - {algorithm}", height=600, **self.default_layout
        )
            ...
            if var.type == VariableType.CATEGORICAL:
                ...
                fig.add_trace(
                    go.Histogram(
                        ...
                        nbinsx=min(
                            30,
                            len(numeric_data.unique()) if var.type == VariableType.DISCRETE else 30,
                        ),
                    ),
                    row=row,
                    col=col,
        ...
        fig = go.Figure(
            data=[
                go.Scatter3d(
                    ...
                    mode="markers",
                    marker=dict(
                        size=5, color=self.colors["primary"], line=dict(color="white", width=0.5)
                    ),
    ...
    def _convert_to_numeric_for_quality(
    ...
    def _calculate_coverage_per_variable(
        self, data: pd.DataFrame, variables: List[Variable]
    ) -> Dict[str, float]:
    ...
    def _calculate_uniformity_scores(
        self, data: pd.DataFrame, variables: List[Variable]
    ) -> Dict[str, float]:
        ...
        for var in variables:
            ...
            if var.type == VariableType.CATEGORICAL:
                ...
                if len(numeric_data) < 2:
                    ...
                    n_bins = min(
                        20, len(numeric_data.unique()) if var.type == VariableType.DISCRETE else 20
                    )
