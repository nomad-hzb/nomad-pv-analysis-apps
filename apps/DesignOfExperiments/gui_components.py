...
class GUIComponents:
    ...
    def create_algorithm_selector(self) -> widgets.Widget:
        ...
        return widgets.VBox([self.algorithm_dropdown, self.algorithm_description])
    ...
    def _on_export_protocol(self, button):
        ...
        if not hasattr(self, "current_samples") or self.current_samples is None:
            self.protocol_display.value = (
                "<p style='color: red;'>No samples to export. Generate samples first.</p>"
            )
            return
        ...
        try:
            ...
            download_link = self.create_download_link(
                protocol_content, "experimental_protocol.txt", "Experimental protocol document"
            )
            ...
            self.protocol_display.value = (
                f"<p style='color: red;'>Protocol export failed: {str(e)}</p>"
            )
    ...
    def _generate_experimental_protocol(self) -> str:
        ...
        for var in self.current_variables:
            ...
            if var.type == VariableType.CONTINUOUS:
                ...
                protocol_lines.append(
                    f"  Range: {var.min_value} - {var.max_value} (step: {var.step_size})"
                )
    ...
    def _on_export_data(self, button):
        ...
        if not hasattr(self, "current_samples") or self.current_samples is None:
            self.export_download_area.value = (
                "<p style='color: red;'>No data to export. Generate samples first.</p>"
            )
            return
        ...
        try:
            if format_type == "csv":
                ...
                download_link = self.create_download_link(
                    csv_content, "doe_samples.csv", "Sample data exported as CSV"
                )

            elif format_type == "json":
                # Export configuration as JSON
                config = {
                    "variables": [var.to_dict() for var in self.current_variables]
                        ...
                        for key, value in self.current_metrics.items():
                            if isinstance(value, dict):
                                for subkey, subvalue in value.items():
                                    metrics_data.append(
                                        {"Metric": f"{key}.{subkey}", "Value": str(subvalue)}
                                    )
                            else:
    ...
    def create_variable_configurator(self) -> widgets.Widget:
        ...
        self.variable_count_label = widgets.HTML(
            value="<b>Variables: 0</b>", layout=widgets.Layout(margin="5px 0")
        )
        ...
        controls = widgets.HBox(
            [add_button, clear_button, widgets.HTML("&nbsp;" * 5), self.variable_count_label]
        )
    ...
    def create_seed_configurator(self) -> widgets.Widget:
        ...
        return widgets.VBox([widgets.HBox([self.random_seed, new_seed_button]), seed_explanation])
    ...
    def create_generation_controls(self) -> widgets.Widget:
        ...
        self._generation_controls = widgets.HBox(
            [self.generate_button, widgets.HTML("&nbsp;" * 3), self.regenerate_button]
        )
    ...
    def create_progress_section(self) -> widgets.Widget:
        ...
        self.progress_label = widgets.HTML(layout=widgets.Layout(visibility="hidden"))

        return widgets.VBox([self.progress_bar, self.progress_label])
    ...
    def create_protocol_section(self) -> widgets.Widget:
        ...
        self.randomize_order = widgets.Checkbox(
            value=True, description="Randomize experimental order", style=self.INPUT_STYLE
        )
        ...
        return widgets.VBox([self.randomize_order, export_protocol_button, self.protocol_display])
    ...
    def create_visualization_controls(self) -> widgets.Widget:
        ...
        controls = widgets.VBox(
            [
                ...
                widgets.HBox([export_plot_button, export_text]),
            ]
        )
    ...
    def create_export_section(self) -> widgets.Widget:
        ...
        return widgets.VBox([self.export_format_dropdown, export_button, self.export_download_area])
    ...
    def _create_variable_widget(self, variable: Optional[Variable] = None) -> widgets.Widget:
        ...
        remove_button = widgets.Button(
            description="", button_style="danger", icon="times", layout=widgets.Layout(width="40px")
        )
        ...
        widget_row = widgets.HBox(
            [name_input, type_selector, param_container, desc_input, remove_button],
        ...
        for widget in self.variable_widgets:
            try:
                ...
                if var_type == VariableType.CONTINUOUS:
                    ...
                    categories = [
                        cat.strip()
                        for cat in widget.categories_input.value.split(",")
                        if cat.strip()
                    ]
                    variable = Variable(
                        name=name, type=var_type, description=description, categories=categories
                    )
    ...
    def update_metrics_display(self, container: widgets.Widget, metrics: Dict[str, Any]):
        ...
        if "min_distance" in metrics:
            ...
            if "distance_uniformity" in metrics:
                html_parts.append(
                    f"<li><b>Distance Uniformity:</b> {metrics['distance_uniformity']:.4f}</li>"
                )
            html_parts.append("</ul>")
        ...
        if "correlation_stats" in metrics:
            ...
            html_parts.append("<ul>")
            html_parts.append(
                f"<li><b>Max Correlation:</b> {corr_stats['max_correlation']:.3f}</li>"
            )
            html_parts.append(
                f"<li><b>Mean Correlation:</b> {corr_stats['mean_correlation']:.3f}</li>"
            )
            html_parts.append("</ul>")
