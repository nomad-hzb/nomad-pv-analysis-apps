...
class ValidationUtils:
    ...
    def validate_numeric_range(
        min_val: float, max_val: float, step: Optional[float] = None
    ) -> Tuple[bool, str]:
    ...
    def sanitize_filename(filename: str) -> str:
        ...
        if len(safe_filename) > 100:
            name, ext = (
                safe_filename.rsplit(".", 1) if "." in safe_filename else (safe_filename, "")
            )
...
class ExperimentalDesignUtils:
    ...
    @staticmethod
    def calculate_design_efficiency(
        samples: pd.DataFrame, variables: List, algorithm: str
    ) -> Dict[str, float]:
    ...
    def recommend_sample_size(
        n_variables: int, algorithm: str, objectives: List[str] = None
    ) -> Dict[str, int]:
    ...
    def assess_design_quality(
        samples: pd.DataFrame, variables: List, metrics: Dict[str, float]
    ) -> Dict[str, str]:
                ...
                if metric in ["min_distance", "overall_coverage"]:
                    ...
                    if value >= thresholds["good"]:
                        ...
                        if metric == "min_distance":
                            recommendations.append(
                                "Consider increasing sample size or using a different algorithm"
                            )
                        elif metric == "overall_coverage":
                            recommendations.append(
                                "Consider adjusting variable ranges or sample distribution"
                            )
                    ...
                    if value <= thresholds["good"]:
                        ...
                        if metric == "distance_uniformity":
                            recommendations.append(
                                "Sample distribution is not uniform - consider different algorithm"
                            )
                        elif metric == "max_correlation":
                            recommendations.append(
                                "High correlation between variables - check for redundancy"
                            )
