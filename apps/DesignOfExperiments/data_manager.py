    ...
    def update_variable(self, variable_name: str, updated_variable: Variable) -> Tuple[bool, str]:
        ...
        for i, var in enumerate(self.variables):
            if var.name == variable_name:
                ...
                if updated_variable.name != variable_name and any(
                    v.name == updated_variable.name for v in self.variables
                ):
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if len(parts) < 3:
                    return (
                        False,
                        f"Line {line_num}: Invalid format. Expected: name,type,params...",
                        [],
                    )
                ...
                if var_type == "continuous":
                    if len(parts) != 4:
                        return (
                            False,
                            f"Line {line_num}: Continuous variables need: name,continuous,min,max",
                            [],
                        )
                    ...
                    try:
                        ...
                        return (
                            False,
                            f"Line {line_num}: Min, max, and step values must be numeric",
                            [],
                        )
                    ...
                    if len(parts) < 4:
                        return (
                            False,
                            f"Line {line_num}: Categorical variables need at least 2 categories",
                            [],
                        )

                    categories = parts[2:]
                    variables.append(
                        {"name": name, "type": "categorical", "categories": categories}
                    )
